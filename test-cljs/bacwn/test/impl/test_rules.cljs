...
(ns bacwn.test.impl.test-rules
  ...
  (:use [fogus.datalog.bacwn.impl.rules :only [predicate-map compute-sip display-rule rules-set apply-rule]]
        [fogus.datalog.bacwn.impl.database :only [datalog-relation datalog-database add-tuples empty-database]]))
...
(deftest test-rule-safety
  (is (thrown-with-msg? js/Error #".*Head vars.*not bound.*"
                        (<- (:fred :x ?x) (:sally :y ?y))))
  (is (thrown-with-msg? js/Error #".*Body vars.*not bound.*negative position.*"
